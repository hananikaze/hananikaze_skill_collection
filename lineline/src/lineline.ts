...
export class Lineline {
  ...
  private loadBuiltinTemplates(): void {
    ...
    for (const f of fs.readdirSync(tplDir).filter(f => f.endsWith('.yaml'))) {
      try {
        const t = YAML.parse(fs.readFileSync(path.join(tplDir, f), 'utf-8')) as PipelineTemplate;
        if (t.name) this.templates.set(t.name, t);
      } catch { /* skip invalid */ }
    }
  }
  ...
  list(filter?: { kind?: string; status?: string; tags?: string[] }): Line[] {
    let lines = this.store.loadAll();
    if (filter?.kind) lines = lines.filter(l => l.kind === filter.kind);
    if (filter?.status) lines = lines.filter(l => l.status === filter.status);
    if (filter?.tags?.length) lines = lines.filter(l => filter.tags!.some(t => l.tags?.includes(t)));
    return lines;
  }
  ...
  private setStatus(id: string, status: Line['status']): void {
    const line = this.store.load(id);
    if (!line) throw new Error(`Line ${id} not found`);
  ...
  pause(id: string): void { this.setStatus(id, 'paused'); }
  resume(id: string): void { this.setStatus(id, 'active'); }
  archive(id: string): void { this.setStatus(id, 'archived'); }

  // ── Timeline ops ──

  addEntry(timelineId: string, event: string, data?: Record<string, any>): TimelineEntry {
    const line = this.store.load(timelineId) as Timeline | undefined;
    if (!line || line.kind !== 'timeline') throw new Error('Timeline not found');
  ...
  advanceStage(pipelineId: string): PipelineStage {
    const line = this.store.load(pipelineId) as Pipeline | undefined;
    if (!line || line.kind !== 'pipeline') throw new Error('Pipeline not found');
    if (line.status === 'done') throw new Error('Pipeline already completed');
  ...
  addStage(pipelineId: string, stage: { name: string; after?: string }): PipelineStage {
    const line = this.store.load(pipelineId) as Pipeline | undefined;
    if (!line || line.kind !== 'pipeline') throw new Error('Pipeline not found');
  ...
  createFromTemplate(templateName: string, name: string): Pipeline {
    ...
    return this.create({ kind: 'pipeline', name, stages: tpl.stages, template: templateName }) as Pipeline;
  }

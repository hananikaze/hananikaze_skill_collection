...
export function createPipeline(name, stages, template, tags, metadata) {
    if (stages.length === 0)
        throw new Error('Pipeline must have at least one stage');
    const now = new Date().toISOString();
    const pipelineStages = stages.map((s, i) => ({
        id: crypto.randomUUID(), name: s.name, status: i === 0 ? 'active' : 'pending',
...
export function completeStage(pl, stageId) {
    ...
    if (stage.status === 'skipped')
        throw new Error('Stage was skipped');
    stage.status = 'done';
    stage.completedAt = new Date().toISOString();
    pl.updatedAt = new Date().toISOString();
    return stage;
}
export function skipStage(pl, stageId) {
    const stage = stageId ? pl.stages.find(s => s.id === stageId) : pl.stages.find(s => s.id === pl.currentStageId);
    if (!stage)
        throw new Error('Stage not found');
    if (stage.status === 'done')
...
export function advanceStage(pl) {
    ...
    if (nextIdx >= pl.stages.length)
        throw new Error('No more stages — pipeline is complete');
...
export function addStage(pl, name, after) {
    const stage = { id: crypto.randomUUID(), name, status: 'pending' };
    if (after) {
        const idx = pl.stages.findIndex(s => s.id === after);
        if (idx === -1)
            throw new Error(`Stage ${after} not found`);
...
export function removeStage(pl, stageId) {
    const idx = pl.stages.findIndex(s => s.id === stageId);
    if (idx === -1)
        throw new Error(`Stage ${stageId} not found`);
    if (pl.currentStageId === stageId)
        throw new Error('Cannot remove active stage');
    const [removed] = pl.stages.splice(idx, 1);
    pl.updatedAt = new Date().toISOString();
    return removed;
}
export function isComplete(pl) {

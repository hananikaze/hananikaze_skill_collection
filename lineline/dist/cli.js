...
function printLine(line) {
    ...
    if (line.kind === 'timeline') {
        ...
        const pl = line;
        if (pl.template)
            console.log(`  template: ${pl.template}`);
...
function findLine(partial) {
    ...
    if (matches.length === 1)
        return matches[0].id;
    if (matches.length > 1) {
        console.error(`Ambiguous ID prefix: ${partial}`);
        process.exit(1);
    }
    console.error(`Line not found: ${partial}`);
    process.exit(1);
}
try {
    switch (cmd) {
        case 'create': {
            ...
            if (!kind || !name) {
                console.error('Usage: lineline create <timeline|pipeline> <name>');
                process.exit(1);
            }
            if (kind === 'timeline') {
        ...
        case 'entry': {
            ...
            if (!event) {
                console.error('Usage: lineline entry <timeline-id> <event>');
                process.exit(1);
            }
        ...
        case 'add-stage': {
            ...
            const name = positional(1);
            if (!name) {
                console.error('Usage: lineline add-stage <pipeline-id> <name> [--after <stage-id>]');
                process.exit(1);
            }
